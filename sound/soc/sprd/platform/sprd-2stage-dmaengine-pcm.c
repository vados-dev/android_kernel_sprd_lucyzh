...
struct sprd_runtime_data {
	...
	int int_pos_update2;
	int dma_pos_pre2;
	uint32_t iram_phy_addr_ap;
	uint32_t iram_phy_addr_dma;
	uint8_t *iram_virt_addr;
	uint32_t iram_data_max_s2_1;
	uint32_t pointer2_step_bytes;
	uint32_t pointer2_step_max;
	uint32_t node_size_level1;
	uint32_t node_cfg_count_2;
};
	...
	struct snd_pcm_substream *substream, u32 linklist_node_size)
{
	...
	for (i = 0; i < chan_cnt; i++) {
		...
		pr_debug("%s chan=%d linklist cfg vir =%#lx, phy =%#lx, node_size =%lu\n ",
	     __func__, i, (unsigned long)sprd_rtd->dma_linklist_cfg_virt[i],
...
pcm_free_dma_linklist_cfg_ddr(struct snd_pcm_substream *substream)
{
	struct snd_pcm_runtime *runtime = substream->runtime;
	struct sprd_runtime_data *sprd_rtd = runtime->private_data;
	uint32_t linklist_node_size = sprd_rtd->linklist_node_size;
	struct snd_soc_pcm_runtime *srtd = substream->private_data;
	struct platform_pcm_priv *priv_data;
	int chan_cnt = 0;
	int i = 0;
					       ...
					       *substream)
{
	struct snd_pcm_runtime *runtime = substream->runtime;
	struct sprd_runtime_data *sprd_rtd = runtime->private_data;
	uint32_t linklist_node_size2 = sprd_rtd->linklist_node_size;
	struct snd_soc_pcm_runtime *srtd = substream->private_data;
	struct platform_pcm_priv *priv_data;
				  ...
				  u32 linklist_node_size2)
{
	...
	for (i = 0; i < chan_cnt; i++) {
		sprd_rtd->dma_config_ptr[i] = devm_kzalloc(dev,
			linklist_node_size, GFP_KERNEL);
		if (!sprd_rtd->dma_config_ptr[i])
			return -ENOMEM;
		sprd_rtd->dma_cb_ptr[i] = devm_kzalloc(dev,
...
static int32_t dmabuffer_reserved_ddr_alloc(struct snd_pcm_substream *substream)
{
...
static int32_t dmabuffer_ddr_alloc(struct snd_pcm_substream *substream,
			u32 buffer_size)
{
	struct snd_dma_buffer *dma_buffer = &substream->dma_buffer;
			      ...
			      struct snd_dma_buffer *dma_buffer)
{
	...
	if (dma_buffer->area && (dma_buffer->dev.type == SNDRV_DMA_TYPE_DEV)) {
		dma_free_writecombine(substream->pcm->card->dev,
			dma_buffer->bytes, dma_buffer->area, dma_buffer->addr);
		dma_buffer->area = NULL;
		dma_buffer->addr = 0;
		dma_buffer->bytes = 0;
	}
...
static int32_t pcm_preallocate_dma_buffer(struct snd_soc_pcm_runtime *srtd,
					  int stream)
{
	int ret = -1;
...
static int32_t pcm_preallocate_dma_buffer_free(struct snd_pcm_substream
					       *substream)
{
	struct snd_dma_buffer *dma_buffer = NULL;
...
static void sprd_pcm_dma_buf_done_level1(void *data)
{
	...
	struct sprd_runtime_data *rtd = runtime->private_data;
	struct snd_soc_pcm_runtime *srtd = substream->private_data;
	uint32_t pointer2_step_bytes = 0;
	uint32_t count_max = 0;
	struct platform_pcm_priv *priv_data = NULL;
...
static int32_t used_hw_chan(struct snd_pcm_substream *substream,
					...
					int ch_cnt)
{
	...
	if (dma_data->used_dma_channel_name2 == NULL) {
		pr_err
...
static int dma_step_cal(uint32_t datawidth, int ch_cnt, int interleaved)
{
	u32 step = 0;
	...
	if (interleaved) {
		...
		step = (datawidth == WORD_WIDTH) ? 4 : 2;
	}
	return step;
}

static void dma_cfg_com(struct sprd_dma_cfg *dma_cfg, uint32_t datawidth,
			uint32_t fragment, uint32_t block, uint32_t transaction,
			uint32_t req_mode, uint32_t irq_mode)
{
	dma_cfg->datawidth = datawidth;
	dma_cfg->fragmens_len = fragment;
	dma_cfg->block_len = block;
	dma_cfg->transcation_len = transaction;
	dma_cfg->req_mode = req_mode;
	dma_cfg->irq_mode = irq_mode;
}

/* iram_count = 0, means ignore iram_count(used by one stage dma) */
static int dma_cfg_list(struct sprd_dma_cfg *dma_cfg_list_base,
			 unsigned long cfg_addr_p, unsigned long cfg_addr_v,
			 unsigned long source_addr_p, uint32_t source_step,
			 unsigned long dst_addr_p, u32 dst_step,
			 u32 wrap_ptr_offset, u32 node_bytes, u32 node_count,
			 u32 iram_count, u32 offset_node_idx, u32 dev_id,
			 enum DMA_LEVEL level)
{
	...
	u32 fact = (dma_cfg_list_base->datawidth == SHORT_WIDTH) ? 2 : 4;
	uint32_t step_num = node_bytes / fact;

	switch (level) {
	case DMA_LEVEL_2:
		source_offset = step_num * source_step;
		dst_offset = 0;
		if (wrap_ptr_offset != 0) {
			/* dst address wrap(iram wrap) */
			wrap_ptr = (dst_addr_p + wrap_ptr_offset);
			wrap_to = dst_addr_p;
		}
	break;
	case DMA_LEVEL_1:
		...
		if (iram_count == 1) {
			...
			source_offset = step_num * source_step;
			/* in our case(iram->vbc) dst_step is 0 */
			dst_offset = step_num * dst_step;
		}
	break;
	...
	pr_info("src_step = %#x, dst_step = %#x, source_offset=%#lx, dst_offset=%#lx, step_num =%#x, fact=%#x, iram_count=%#x node_bytes=%#x\n",
	     source_step, dst_step, source_offset, dst_offset, step_num, fact,
	     iram_count, node_bytes);
	for (i = 0; i < node_count; i++) {
		cfg = dma_cfg_list_base + i;
		memcpy(cfg, dma_cfg_list_base, sizeof(struct sprd_dma_cfg));
		cfg->src_addr =
		    (unsigned long)(source_addr_p +
				    source_offset * ((i + offset_node_idx) %
						     node_count));
		cfg->des_addr =
		    (unsigned long)(dst_addr_p +
				    dst_offset * ((i + offset_node_idx) %
						  node_count));
		cfg->src_step = source_step;
		cfg->des_step = dst_step;
		cfg->link_cfg_v = (unsigned long)cfg_addr_v;
		cfg->link_cfg_p = (unsigned long)cfg_addr_p;
		cfg->dev_id = (u32) dev_id;
		cfg->wrap_ptr = wrap_ptr;
		cfg->wrap_to = wrap_to;
		cfg->is_end = 0;
	}
	cfg->is_end = 2;
						  ...
						  void *data)
{
	...
	desp = chn->device->device_prep_dma_memcpy(chn, 0, 0, 0, flag);
	if (!desp) {
...
static int dma_cfg_set(struct sprd_dma_cfg *dma_cfg,
			unsigned long source_addr_p, unsigned long dst_addr_p,
			u32 wrap_ptr_offset, unsigned long cfg_addr_p,
			unsigned long cfg_addr_v, u32 src_step, u32 dst_step,
			u32 datawidth, u32 fragments_len, u32 block_len,
			u32 translen, u32 dev_id, u32 irq_mode,
			u32 iram_count, u32 node_count, u32 offset_node_idx,
			u32 node_bytes, enum DMA_LEVEL level)
{
	int i = 0;
	int ret = 0;

	dma_cfg_com(dma_cfg, datawidth, fragments_len, block_len, translen,
		    FRAG_REQ_MODE, irq_mode);
	pr_info("datawidth=%d, fragmens_len=%#x, block_len =%#x, trans_len=%#x req_mode=%d, irq_mode=%d\n",
	     dma_cfg->datawidth, dma_cfg->fragmens_len, dma_cfg->block_len,
	     dma_cfg->transcation_len, dma_cfg->req_mode, dma_cfg->irq_mode);
	ret = dma_cfg_list(dma_cfg, cfg_addr_p, cfg_addr_v, source_addr_p,
		     src_step, dst_addr_p, dst_step, wrap_ptr_offset,
		     node_bytes, node_count, iram_count, offset_node_idx,
		     dev_id, level);
	if (ret < 0)
		return ret;
	for (i = 0; i < node_count; i++) {
		pr_info("src_addr %#lx, cfg->des_addr %#lx, link_cfg_v %#lx, link_cfg_p %#lx, src_step %d, des_step %d, dev_id %#x, is_end %d, node_idx %d, node_count %d, linklist_ptr %#lx, wrap_ptr %#x, wrap_to %#x\n",
		     dma_cfg[i].src_addr, dma_cfg[i].des_addr,
		     dma_cfg[i].link_cfg_v, dma_cfg[i].link_cfg_p,
		     dma_cfg[i].src_step, dma_cfg[i].des_step,
		     dma_cfg[i].dev_id, dma_cfg[i].is_end, i, node_count,
		     dma_cfg[i].linklist_ptr, dma_cfg[i].wrap_ptr,
		     dma_cfg[i].wrap_to);
	}
				   ...
				   struct sprd_runtime_data *sprd_rtd)
{
	struct snd_soc_pcm_runtime *srtd = substream->private_data;
	struct platform_pcm_priv *priv_data = NULL;
	uint32_t phy_addr_ap;
	uint32_t phy_addr_dma;
	uint32_t offset;
	uint8_t *virt_addr;
	uint32_t linklist_node_size = SPRD_AUDIO_DMA_NODE_SIZE;
	int normal = 0;
	int chan_cnt = 0;
	int i = 0;
	...
	if (normal == PLY_NORMAL) {
		...
		virt_addr = (uint8_t *) (priv_data->iram_normal_virt_addr);
		offset =
		...
		virt_addr = (uint8_t *) (priv_data->iram_deepbuf_virt_addr);
		offset =
					 ...
					 struct snd_pcm_hw_params *params)
{
	...
	*/
	...
	linklist_node_size_1 = node_cfg_count_1 * sizeof(struct sprd_dma_cfg);
	linklist_node_size_2 = node_cfg_count_2 * sizeof(struct sprd_dma_cfg);
	...
	pr_info("hw_chan=%d, hw_chan2=%d totsize=%#zx(%#zx frames), period=iram_size=%#zx(%#zx frames), width_in_bytes=%d, channels=%d\n",
	     rtd->hw_chan, rtd->hw_chan2, totsize,
	     totsize / width_in_bytes / channels, period,
	     period / width_in_bytes / channels, width_in_bytes,
	     channels);
	...
	for (i = 0; i < ch_cnt_1; i++) {
		...
		if (i == 0) {
			if (srtd->cpu_dai->id == VBC_DAI_NORMAL) {
				flag_1 =
				    ...
				    DMA_GROUP1_SRC | DMA_MUTL_TRANS_DONE;
				...
				flag_1 =
				    DMA_CFG_FLAG | DMA_HARDWARE_FLAG |
				    DMA_GROUP2_SRC | DMA_MUTL_TRANS_DONE;
			...
			flag_1 = DMA_CFG_FLAG | DMA_HARDWARE_FLAG;

		fragments_len_1 =
		    dma_data->desc.fragmens_len * (SHORT_WIDTH ==
						   dma_data->desc.
						   datawidth ? 2 : 4);
		...
		/* level 1 */
		ret = dma_cfg_set(rtd->dma_config_ptr[i], source_addr_p_1,
			    dst_addr_p_1, 0,
			    (unsigned long)rtd->dma_linklist_cfg_phy[i],
			    (unsigned long)rtd->dma_linklist_cfg_virt[i],
			    src_step_1, dst_step_1, dma_data->desc.datawidth,
			    fragments_len_1, block_len_1, trans_len_1,
			    dma_data->channels[i], irq_mode_1, iram_count_1,
			    node_cfg_count_1, 0, nodebytes_1,
			    DMA_LEVEL_1);
		if (ret < 0)
			goto hw_param_err;
		rtd->dma_tx_des[i] =
		    dma_cfg_hw(rtd->dma_chn[i], rtd->dma_config_ptr[i], flag_1,
			       callback_1, callback_param_1);
	}
	...
	if (srtd->cpu_dai->id == VBC_DAI_NORMAL) {
		flag_2 = DMA_CFG_FLAG | DMA_HARDWARE_FLAG | DMA_GROUP1_DST;
		pr_info("cpu_dai[%d] use group-1\n", srtd->cpu_dai->id);
	} else if (srtd->cpu_dai->id == VBC_DAI_DEEP_BUF) {
		flag_2 = DMA_CFG_FLAG | DMA_HARDWARE_FLAG | DMA_GROUP2_DST;
		pr_info("cpu_dai[%d] use group-2\n", srtd->cpu_dai->id);
	} else {
		pr_err("%s %d failed unknown cpu_dai_id=%d\n",
		       __func__, __LINE__, srtd->cpu_dai->id);
	...
	ret = dma_cfg_set(rtd->dma_cfg_buf2, source_addr_p_2, dst_addr_p_2,
		    ...
		    (unsigned long)rtd->dma_linklist_cfg_virt2, src_step_2,
		    dst_step_2, dma_data->desc2.datawidth, fragments_len_2,
		    block_len_2, trans_len_2, 0, irq_mode_2, 0,
		    node_cfg_count_2, offset_node_idx_2, nodebytes_2,
		    DMA_LEVEL_2);
	if (ret < 0)
		goto hw_param_err;
	rtd->dma_tx_des2 =
	    dma_cfg_hw(rtd->dma_chn2, rtd->dma_cfg_buf2, flag_2, callback_2,
		       callback_param_2);
				   ...
				   struct snd_pcm_hw_params *params)
{
	...
	uint32_t flag = DMA_CFG_FLAG | DMA_HARDWARE_FLAG;
	void *callback = NULL;
	u32 fragments_len = 0;
	u32 block_len = 0;
	u32 trans_len = 0;
	...
	if (dma_buffer->dev.type == SNDRV_DMA_TYPE_DEV_IRAM) {
		...
		if (is_playback == true) {
			if (srtd->cpu_dai->id == VBC_DAI_NORMAL) {
				pcm_set_dma_linklist_cfg_iram_s1(substream,
					linklist_node_size,
					PLY_NORMAL);
			} else {
				pcm_set_dma_linklist_cfg_iram_s1(substream,
		...
		linklist_node_size = node_count * sizeof(struct sprd_dma_cfg);
	...
	for (i = 0; i < ch_cnt; i++)
		pr_info("dma_buff_phys[%d] %#x\n", i,
			(uint32_t) dma_buff_phys[i]);
	fragments_len = dma_data->desc.fragmens_len *
	    ((dma_data->desc.datawidth == SHORT_WIDTH) ? 2 : 4);
	...
	for (i = 0; i < ch_cnt; i++) {
		...
		irq_mode =
		    (params->
		     flags & SNDRV_PCM_HW_PARAMS_NO_PERIOD_WAKEUP) ? NO_INT :
		    BLK_DONE;
		ret = dma_cfg_set(rtd->dma_config_ptr[i], source_addr_p,
			    dst_addr_p, 0,
			    (unsigned long)rtd->dma_linklist_cfg_phy[i],
			    (unsigned long)rtd->dma_linklist_cfg_virt[i],
			    src_step, dst_step, dma_data->desc.datawidth,
			    fragments_len, block_len, trans_len,
			    dma_data->channels[i], irq_mode, 0, node_count, 0,
			    node_bytes, DMA_LEVEL_ONESTAGE);
		if (ret < 0) {
			normal_dma_protect_mutex_unlock(substream);
			goto hw_param_err;
		}
		tmp_tx_des =
			dma_cfg_hw(rtd->dma_chn[i], rtd->dma_config_ptr[i],
				   flag, callback, rtd->dma_cb_ptr[i]);
		if (!tmp_tx_des) {
			normal_dma_protect_mutex_unlock(substream);
...
static void init_iram_data_2stage(struct snd_pcm_substream *substream)
{
	...
	struct sprd_runtime_data *rtd = runtime->private_data;
	struct sprd_pcm_dma_params *dma = rtd->params;
	uint8_t *iram_addr_virt = rtd->iram_virt_addr;
	uint8_t *ddr_addr_virt = runtime->dma_area;
	uint32_t transction2 = dma->desc2.transcation_len;
...
static uint32_t get_dma_used_total_cnt(struct platform_pcm_priv *priv_data,
				       int normal)
{
	...
	if (priv_data->chan_cnt[normal] && priv_data->width_in_bytes[normal])
...
static inline uint32_t get_dma_level1_int_count(struct snd_pcm_substream
						*substream, int normal)
{
	uint32_t offset = (normal == PLY_NORMAL) ? 0 : 8;
	struct snd_soc_pcm_runtime *srtd = substream->private_data;
	struct platform_pcm_priv *priv_data = NULL;
						 ...
						 *substream)
{
	...
	uint32_t cm4_dma_level1_buf_done_count = 0;
	struct platform_pcm_priv *priv_data = NULL;
	...
	if (priv_data->dma_2stage_level_1_int_source ==
		...
		if (priv_data->proc[PROC_POINTER_LOG] != 0) {
			pr_info("cpu_dai[%d]cm4_arm7_dma_level1_buf_done_count =%u, app_pointer=%#lx, count_max=%d\n",
			     srtd->cpu_dai->id, cm4_dma_level1_buf_done_count,
			 ...
			 struct vm_area_struct *vma)
{
	...
	pr_info("%s line[%d] dma_addr=%#zx dma_area= %p, runtime->dma_bytes = %#zx, vm_start %#lx, vm_end %#lx, end-start =%#lx\n",
	     __func__, __LINE__, (size_t)runtime->dma_addr, runtime->dma_area,
	     runtime->dma_bytes, vma->vm_start, vma->vm_end,
...
int32_t platform_pcm_parse_dt(struct device_node *node,
			      struct platform_pcm_priv *priv_data)
{
	int32_t ret = 0;
	...
	if (priv_data->iram_size > 0) {
		priv_data->iram_virt_addr =
		    (char *)devm_ioremap_nocache(platform->dev,
						 ...
						 priv_data->iram_size);
		if (priv_data->iram_virt_addr == NULL) {
			pr_err("%s %d failed\n", __func__, __LINE__);
			return -1;
		}
