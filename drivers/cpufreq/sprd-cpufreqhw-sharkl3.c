...
#ifdef SPRD_CPUFREQ_I2C_LOCK
static int sprd_cpufreqhw_i2c_clr_nack(void)
{
	int ret;
	...
	msg[0].addr = cpufreqhw->i2c_client->addr;
	msg[0].flags = 0;
	msg[0].len = 1;
	msg[0].buf = &start;

	msg[1].addr = cpufreqhw->i2c_client->addr;
	msg[1].flags = I2C_M_RD;
	msg[1].len = 1;
	msg[1].buf = buf;

	ret = __i2c_transfer(cpufreqhw->i2c_client->adapter, &msg[0], 2);

	P_INF("cpufreqhw_i2c_clr_nack ret %d recv 0x%x", ret, buf[0]);
	ret = (ret < 0 ? ret : 0);
