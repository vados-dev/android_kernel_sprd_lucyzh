...
void panel_power_ctrl(struct power_sequence *seq)
{
	...
	if (g_zte_ctrl_pdata->zte_lcd_ctrl->lcd_powerdown_for_shutdown) {
		for (i = 0; i < items; i++) {
			gpio_direction_output(timing[i].gpio, 0);
		}
		pr_info("[SPRD_LCD]panel_power_ctrl:shutdonw for TP and lcd power off pin\n");
...
static void panel_suspend(struct panel_device *pd)
{
	...
	if (suspend_tp_need_awake() && g_zte_ctrl_pdata->zte_lcd_ctrl->have_tp_gesture_power_off_seq) {
		pr_info("[SPRD_LCD] tp gesture power off sequence\n");
		panel_power_ctrl(&panel->pwr_off_tp_gesture_seq);
	} else {
...
static struct panel_info *of_parse_panel(struct device_node *panel_node)
{
	...
	rc = of_property_read_u32(panel_node, "fps-v2", &temp);
	#else
