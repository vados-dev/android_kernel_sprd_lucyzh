...
void clr_vsp_interrupt_mask(void)
{
	int cmd = 0;
	...
	if (vsp_hw_dev.version < SHARKL3) {
		...
		/* clear vsp int */
		writel_relaxed(0x3ffff, (vsp_glb_reg_base + VSP_INT_CLR_OFF));
	...
	if (vsp_hw_dev.version == SHARKL3) {
		...
		/* clear vsp int */
		writel_relaxed(0xfff, (vsp_glb_reg_base + VSP_INT_CLR_OFF));
		writel_relaxed(0xff, (sprd_vsp_base + VSP_MMU_INT_CLR_OFF));
	}
