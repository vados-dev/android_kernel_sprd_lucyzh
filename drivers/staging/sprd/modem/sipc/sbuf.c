...
/* *
 * sbuf_update_wakelock - according sbuf state, update sbuf wake lock
 * @state: the wake lock state, 1: means have got a long wake lock,
 * only state is 1 ,we can do below function,
 * ortherwise system can't enter to sleep, 0: do nothing
 * @ need:  whether need wake lock, 1: update wake lock time to 20ms,
 * 0: unlock it immediately
 * @lock: wake lock point
 * @name: wake lock name
 */
#define sbuf_update_wakelock(state, need, lock, name)	\
({	\
	if (state) {	\
		state = 0;	\
		if (need) {	\
			pr_debug("sbuf %s : wake_lock hz/50!\n", name);	\
			wake_lock_timeout(lock, HZ / 50);	\
		} else {	\
...
static struct task_struct *sbuf_wait_get_task(wait_queue_t *pos, u32 *b_select)
{
...
static void sbuf_record_rdwt_owner(struct sbuf_ring *ring, int b_rx)
{
	...
	if (b_add) {
		...
		if (cnt == MAX_RECORD_CNT) {
			temp = list_first_entry(owner_list, struct name_node, list);
			list_del(&temp->list);
			kfree(temp);
		}
...
int sbuf_create(u8 dst, u8 channel, u32 bufnum, u32 txbufsize, u32 rxbufsize)
{
	...
	for (i = 0; i < bufnum; i++) {
		...
		sprintf(ring->tx_wakelock_name,
			"sbuf-%d-%d-%d-tx",
			dst, channel, i);
		wake_lock_init(&ring->tx_wake_lock,
			       WAKE_LOCK_SUSPEND,
			       ring->tx_wakelock_name);
		sprintf(ring->rx_wakelock_name,
			"sbuf-%d-%d-%d-rx",
			dst, channel, i);
		wake_lock_init(&ring->rx_wake_lock,
			       WAKE_LOCK_SUSPEND,
			       ring->rx_wakelock_name);
	}
...
EXPORT_SYMBOL(sbuf_set_no_need_wake_lock);
...
#endif
			wake_lock_destroy(&sbuf->rings[i].tx_wake_lock);
			wake_lock_destroy(&sbuf->rings[i].rx_wake_lock);
		}
		kfree(sbuf->rings);
	}
	       ...
	       void *buf, u32 len, int timeout)
{
	...
	sbuf_update_wakelock(ring->tx_wakelock_state,
			     ring->need_wake_lock,
			     &ring->tx_wake_lock,
			     ring->tx_wakelock_name
			    );
	      ...
	      void *buf, u32 len, int timeout)
{
	...
	sbuf_update_wakelock(ring->rx_wakelock_state,
			     ring->need_wake_lock,
			     &ring->rx_wake_lock,
			     ring->rx_wakelock_name
			    );
				 ...
				 struct sbuf_mgr *sbuf, int task_type)
{
	...
	for (n = 0;  n < sbuf->ringnr;	n++) {
		...
		list_for_each_entry(pos, &phead->task_list, task_list) {
			task = sbuf_wait_get_task(pos, &b_select);
			if (!task)
				continue;
