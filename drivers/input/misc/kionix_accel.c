...
static s32 g_sensor_cali_write(struct file *filp, const char __user *buff,
							size_t len, loff_t *data)
{
	...
	pr_info(" wellcome to g_sensor_cali_write !!");

	if (len != 12) {
		pr_info("g_sensor_cali_write need 12 char !!");
		return -EPERM;
	}
	...
	for (i = 0; i < G_SENSOR_ALL; i++) {
		pr_info("g_sensor_cali_write [%d]:%d ", i, g_sensor_compensation[i]);
	}
...
static s32 g_sensor_cali_read(struct file *file, char __user *buf, size_t size,
							loff_t *ppos)
{
	int i = 0;
	int j = 0;

	pr_info(" wellcome to g_sensor_cali_read !!\n");

	if (g_acceld == NULL) {
		pr_err("g_acceld is NULL !!\n");
		return -EPERM;
	}

	memset(g_sensor_array, 0, sizeof(int) * G_SENSOR_ALL * G_SENSOR_CAIL_NUM);

	if (kionix_accel_enable(g_acceld) != 0) {
		pr_err("can't enable sensor data!!\n");
		return -EPERM;
	}
	...
	if (point_num == 0) {
		pr_err("no sensor data!!\n");
		return -EPERM;
	}
	pr_info("sensor data ok!!\n");

	if (kionix_accel_disable(g_acceld) != 0) {
		pr_err("can't disable sensor data!!\n");
		return -EPERM;
	}
	...
	for (i = 0; i < G_SENSOR_ALL; i++) {
		pr_info("end the cali g_sensor_compensation[%d]:[%d]\n", i, g_sensor_compensation[i]);
	}
						...
						const int token_nr)
{
	...
	strlcpy(buf2, buf, (count + 1) * sizeof(buf));
										...
										unsigned int poll_interval)
{
	...
	case ACCEL_GRP4_ODR0_781:
		pr_info("kionix ODR = 0.781 Hz\n");
		break;
	case ACCEL_GRP4_ODR1_563:
		pr_info("kionix ODR = 1.563 Hz\n");
		break;
	case ACCEL_GRP4_ODR3_125:
		pr_info("kionix ODR = 3.125 Hz\n");
		break;
	case ACCEL_GRP4_ODR6_25:
		pr_info("kionix ODR = 6.25 Hz\n");
		break;
	case ACCEL_GRP4_ODR12_5:
		pr_info("kionix ODR = 12.5 Hz\n");
		break;
	case ACCEL_GRP4_ODR25:
		pr_info("kionix ODR = 25 Hz\n");
		break;
	case ACCEL_GRP4_ODR50:
		pr_info("kionix ODR = 50 Hz\n");
		break;
	case ACCEL_GRP4_ODR100:
		pr_info("kionix ODR = 100 Hz\n");
		break;
	case ACCEL_GRP4_ODR200:
		pr_info("kionix ODR = 200 Hz\n");
		break;
	case ACCEL_GRP4_ODR400:
		pr_info("kionix ODR = 400 Hz\n");
		break;
	case ACCEL_GRP4_ODR800:
		pr_info("kionix ODR = 800 Hz\n");
		break;
	case ACCEL_GRP4_ODR1600:
		pr_info("kionix ODR = 1600 Hz\n");
		break;
	default:
		pr_info("kionix Unknown ODR\n");
		break;
	}
									...
									const char *buf, size_t count)
{
	...
	err = kstrtouint((const char *)buf, 10,
...
static ssize_t kionix_accel_name_show(struct device *dev,
									struct device_attribute *attr, char *buf)
{
	return snprintf(buf, 1024, "%s", KIONIX_ACCEL_VENDOR_NAME);
}

static ssize_t kionix_accel_chip_info(struct device *dev,
									struct device_attribute *attr, char *buf)
{
	return snprintf(buf, 1024, "%s", "kionix_accel_grp2");
}
...
static DEVICE_ATTR(chip_info,     S_IRUGO, kionix_accel_chip_info, NULL);
...
#ifdef CONFIG_GET_DEVICES_VENDOR_INFO
		strlcpy(g_sensor_ic_info, "KXTE9", 6);
#endif
...
#ifdef CONFIG_GET_DEVICES_VENDOR_INFO
		strlcpy(g_sensor_ic_info, "KXTF9", 6);
#endif
		KMSGINF(&acceld->client->dev,
...
#ifdef CONFIG_GET_DEVICES_VENDOR_INFO
		strlcpy(g_sensor_ic_info, "KXCJ9-1008", 11);
#endif
		KMSGINF(&acceld->client->dev,
...
#ifdef CONFIG_GET_DEVICES_VENDOR_INFO
		strlcpy(g_sensor_ic_info, "KXTJ2-1009", 11);
#endif
		KMSGINF(&acceld->client->dev,
...
#ifdef CONFIG_GET_DEVICES_VENDOR_INFO
		strlcpy(g_sensor_ic_info, "KXCJK-1013", 11);
#endif
		KMSGINF(&acceld->client->dev,
...
#ifdef CONFIG_GET_DEVICES_VENDOR_INFO
		strlcpy(g_sensor_ic_info, "KX023", 6);
#endif
		KMSGINF(&acceld->client->dev,
...
static s32 kionix_tool_write(struct file *filp, const char __user *buff,
							 size_t len, loff_t *data)
{
	return 1;
}
								...
								unsigned long arg)
{
	...
	case KIONIX_ACCEL_IOCTL_SET_ENABLE: {
		pr_info("KIONIX_ACCEL_IOCTL_SET_ENABLE\n");
		...
		if (g_acceld == NULL) {
			pr_info("kionix_acc_misc_ioctl error [NULL==g_acceld] !!");
			return -EINVAL;
		}
	...
	case KIONIX_ACCEL_IOCTL_GET_ENABLE:
		pr_info("KIONIX_ACCEL_IOCTL_GET_ENABLE\n");
	...
	case KIONIX_ACCEL_IOCTL_SET_DELAY:
		...
		pr_info("KIONIX_ACCEL_IOCTL_SET_DELAY %lu\n", delay);
		/*
		 * Set current interval to the greater of the minimum interval or
		 * the requested interval
		 */
		acceld->poll_interval =
						max((unsigned int)delay,
							acceld->accel_pdata.min_interval);
		acceld->poll_delay = msecs_to_jiffies(acceld->poll_interval);
		err =
			acceld->kionix_accel_update_odr(acceld,
	...
	case KIONIX_ACC_IOCTL_GET_CHIP_ID: {
		...
		snprintf(devinfo, 1024, "%s, %#x", DEVICE_INFO, devid);
							...
							const struct i2c_device_id *id)
{
	...
	g_chip_id = err;
#ifdef CONFIG_GET_DEVICES_VENDOR_INFO
	strlcpy(g_sensor_vendor_info, "Kionix_1st", 11);
#endif
	...
	if (proc_dir == NULL)
		...
		proc_entry = proc_create("accelinfo", 0644, proc_dir, &kionix_ops);
	...
	KMSGINF(&client->dev, "<%s> probe successfully L%d.\n", __func__,
			__LINE__);
	g_sensor_proc_entry =
		proc_create(G_SENSOR_CALI_PROCNAME, S_IRUGO, NULL,
					&g_sensor_cali_ops);

	if (g_sensor_proc_entry == NULL) {
		pr_info("kionix_accel_init can't do proc_create");
	}
