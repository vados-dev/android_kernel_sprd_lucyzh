...
/* PROC-END */
#endif
extern void hiamx_tpd_register_fw_class(void);
extern int himax_get_tp_vendor_info(void);
...
int adf_notifier_callback(struct notifier_block *nb, unsigned long action, void *data);
...
extern void himax_log_touch_event_detail(struct himax_ts_data *ts, int x, int y, int w, int loop_i,
					 uint8_t EN_NoiseFilter, int touched, uint16_t old_finger);
...
int himax_input_register(struct himax_ts_data *ts)
{
	...
	input_set_abs_params(ts->input_dev, ABS_MT_PRESSURE, ts->pdata->abs_pressure_min, ts->pdata->abs_pressure_max,
			     ts->pdata->abs_pressure_fuzz, 0);
	input_set_abs_params(ts->input_dev, ABS_MT_WIDTH_MAJOR, ts->pdata->abs_width_min, ts->pdata->abs_width_max,
			     ts->pdata->abs_pressure_fuzz, 0);
#endif
...
static int i_get_FW(void)
{
	...
	if (image != NULL) {
		...
		I("%s: i_CTPM_FW [0] = %02X, [1] = %02X, [2] = %02X, [3] = %02X\n", __func__, i_CTPM_FW[0],
		  i_CTPM_FW[1], i_CTPM_FW[2], i_CTPM_FW[3]);
	} else {
		I("%s: i_CTPM_FW = NULL\n", __func__);
		return OPEN_FILE_FAIL;
	}
...
static void himax_chip_monitor_function(struct work_struct *work)	/* for ESD solution */
{
	...
	if (g_chip_monitor_data->HX_CHIP_POLLING_COUNT >= (g_chip_monitor_data->HX_POLLING_TIMES - 1)) {
...
void himax_wake_check_func(void)
{
	...
	if (ret_event) {
		...
		I("gest_up_x= %d, gest_up_y= %d, gest_down_x= %d, gest_down_y= %d\n", gn_gesture_coor[8],
		  gn_gesture_coor[9], gn_gesture_coor[10], gn_gesture_coor[11]);
		I("gest_left_x= %d, gest_left_y= %d, gest_right_x= %d, gest_right_y= %d\n", gn_gesture_coor[12],
		  gn_gesture_coor[13], gn_gesture_coor[14], gn_gesture_coor[15]);
#endif
	}
}
...
int himax_report_data_init(void)
{
	...
	if (hx_touch_data->raw_cnt_rmd != 0x00) { /* more than 4 fingers */
		hx_touch_data->rawdata_size =
		    cal_data_len(hx_touch_data->raw_cnt_rmd, ic_data->HX_MAX_PT, hx_touch_data->raw_cnt_max);
	...
	if ((ic_data->HX_TX_NUM * ic_data->HX_RX_NUM + ic_data->HX_TX_NUM +
		...
		hx_touch_data->rawdata_frame_size =
		    (ic_data->HX_TX_NUM * ic_data->HX_RX_NUM + ic_data->HX_TX_NUM +
		     ic_data->HX_RX_NUM) / hx_touch_data->rawdata_size;
	else
		hx_touch_data->rawdata_frame_size =
		    (ic_data->HX_TX_NUM * ic_data->HX_RX_NUM + ic_data->HX_TX_NUM +
		     ic_data->HX_RX_NUM) / hx_touch_data->rawdata_size + 1;
		...
		for (loop_i = 0; loop_i < ts->nFinger_support; loop_i++) {
			...
			if (x >= 0 && x <= ts->pdata->abs_x_max && y >= 0 && y <= ts->pdata->abs_y_max) {
				...
				if ((ts->debug_log_level & BIT(3)) > 0) {
					himax_log_touch_event_detail(ts, x, y, w, loop_i, EN_NoiseFilter, HX_FINGER_ON,
								     old_finger);
				}
#ifndef	HX_PROTOCOL_A
			...
			/* report coordinates */
				...
				if ((ts->debug_log_level & BIT(3)) > 0) {
					himax_log_touch_event_detail(ts, x, y, w, loop_i, Last_EN_NoiseFilter,
				...
				if (((ts->pre_finger_mask >> loop_i) & 1) == 1) {
					if (ts->useScreenRes) {
						...
						I("status:%X, Raw:F:%02d Up, X:%d, Y:%d, N:%d\n", 0, loop_i + 1,
						  ts->pre_finger_data[loop_i][0], ts->pre_finger_data[loop_i][1],
						  Last_EN_NoiseFilter);
					}
				}
			}
...
{
	struct himax_ts_data *ts = container_of(work, struct himax_ts_data, resume_work);
...
#if defined(CONFIG_ADF)
static void himax_adf_register(struct work_struct *work)
{
	int ret = 0;
	...
	ts->adf_fb_notif.notifier_call = adf_notifier_callback;
	/* make sure we're the first to suspend/resume */
	ts->adf_fb_notif.priority = 1000;
	ret = adf_register_client(&ts->adf_fb_notif);
	if (ret < 0)
		E("register adf notifier fail, cannot sleep when screen off\n");
	else
		I("tp register adf notifier succeed\n");
}
#elif defined(CONFIG_FB)
static void himax_fb_register(struct work_struct *work)
{
	...
	ts->adf_fb_notif.notifier_call = fb_notifier_callback;
	ret = fb_register_client(&ts->adf_fb_notif);
	if (ret)
...
FW_force_upgrade:
	...
	if (getMutualBuffer() == NULL) {
		...
		goto err_mutual_buffer_allocate_failed;
	}
#ifdef HX_TP_PROC_2T2R
	if (Is_2T2R) {
		...
		if (getMutualBuffer_2() == NULL) {
			...
			goto err_mutual_buffer2_allocate_failed;
		}
	}
#endif
#endif
...
#if defined(CONFIG_ADF)
	INIT_DELAYED_WORK(&ts->work_att, himax_adf_register);
#elif defined(CONFIG_FB)
	INIT_DELAYED_WORK(&ts->work_att, himax_fb_register);
#endif
	queue_delayed_work(ts->himax_att_wq, &ts->work_att, msecs_to_jiffies(15000));
	INIT_WORK(&ts->resume_work, himax_resume_work);
	ts->tp_resume_workqueue = create_singlethread_workqueue("himax_resume_work");
	if (!ts->tp_resume_workqueue) {
		err = -ENOMEM;
		goto err_create_resume_workqueue;
	}
#endif
...
#endif
#if defined(CONFIG_FB) || defined(CONFIG_ADF)
	cancel_work_sync(&ts->resume_work);
	destroy_workqueue(ts->tp_resume_workqueue);
err_create_resume_workqueue:
...
#ifdef HX_TP_PROC_2T2R
err_mutual_buffer2_allocate_failed:
	freeMutualBuffer_2();
#endif
#ifdef HX_TP_PROC_DIAG
err_mutual_buffer_allocate_failed:
	freeMutualBuffer();
#endif
err_detect_failed:
#ifdef HX_ZERO_FLASH
...
#if defined(CONFIG_ADF) || defined(CONFIG_FB)
#if defined(CONFIG_ADF)
	if (adf_unregister_client(&ts->adf_fb_notif))
		E("Error occurred while unregistering adf_notifier.\n");
#elif defined(CONFIG_FB)
	if (fb_unregister_client(&ts->adf_fb_notif))
