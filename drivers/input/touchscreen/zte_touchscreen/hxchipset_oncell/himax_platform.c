...
void himax_vk_parser(struct device_node *dt, struct himax_i2c_platform_data *pdata)
{
	u32 data = 0;
	uint8_t cnt = 0, i = 0;
	uint32_t coords[4] = { 0 };
...
int himax_int_register_trigger(struct i2c_client *client)
{
	...
	if (ic_data->HX_INT_IS_EDGE) {
		...
		ret =
		    request_threaded_irq(client->irq, NULL, himax_ts_thread, IRQF_TRIGGER_FALLING | IRQF_ONESHOT,
					 client->name, ts);
	} else {
		I("%s level trigger low\n ", __func__);
		ret =
		    request_threaded_irq(client->irq, NULL, himax_ts_thread, IRQF_TRIGGER_LOW | IRQF_ONESHOT,
					 client->name, ts);
	}
	return ret;
}
...
int adf_notifier_callback(struct notifier_block *nb, unsigned long action, void *data)
{
	...
	struct himax_ts_data *ts = container_of(nb, struct himax_ts_data, adf_fb_notif);
	...
	case DRM_MODE_DPMS_ON:
		ret = queue_work(ts->tp_resume_workqueue, &ts->resume_work);
		if (!ret) {
			E("start tpd_resume_wq failed\n");
			return ret;
		}
		break;
...
int fb_notifier_callback(struct notifier_block *self, unsigned long event, void *data)
{
	...
	int *blank;
	struct himax_ts_data *ts = container_of(self, struct himax_ts_data, adf_fb_notif);
	int ret = 0;
