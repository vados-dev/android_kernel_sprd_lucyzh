...
static int fts_parse_dt(struct device *dev, struct fts_ts_platform_data *pdata)
{
	...
	if (pdata->have_key) {
		...
		FTS_INFO("combKey X Coords %x %x %x\n", pdata->comb_key_x_coord[0],
...
#if FTS_PSENSOR_EN
	if (ts_data->psensor_pdata->tp_psensor_enable == true) {
		FTS_INFO("%s: psensor_enable.\n", __func__);
		ts_data->psensor_pdata->sensors_enable(ts_data->psensor_pdata, true);
		ts_data->psensor_pdata->tp_psensor_enable = false;
	}
#endif
