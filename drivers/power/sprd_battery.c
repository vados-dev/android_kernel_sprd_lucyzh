...
static bool has_set_shipmode = false;
...
void sprdfgu_report_show(int *cc, int *tm, int *avg_cur);
...
static void sprdbat_current_adjust_by_status(uint32_t status)
{
	...
	if (status == STATUS_T0_TO_T1 || status == STATUS_T1_TO_T2) {
		SPRDBAT_DEBUG("jeita status = %d, batt cool!\n", status);
		sprdbat_change_module_state(SPRDBAT_CHGR_COOL_E);
	} else if (status == STATUS_T3_TO_T4) {
		SPRDBAT_DEBUG("jeita status = %d, batt warm!\n", status);
		sprdbat_change_module_state(SPRDBAT_CHGR_WARM_E);
	} else if (status == STATUS_T2_TO_T3) {
		if (sprdbat_data->bat_info.bat_health == POWER_SUPPLY_HEALTH_COOL ||
			sprdbat_data->bat_info.bat_health == POWER_SUPPLY_HEALTH_WARM)
...
static void sprdbat_temp_monitor(void)
{
	...
	if (cur_temp_status != sprdbat_data->last_temp_status) {
		if (cur_temp_status <= STATUS_T1_TO_T2 || cur_temp_status >= STATUS_T3_TO_T4) {
			SPRDBAT_DEBUG("polling fast %d!\n", poll_time_fast);
			sprdbat_data->battery_work_cycle_s = poll_time_fast;
			sprdchg_timer_op->timer_disable();
			sprdchg_timer_op->timer_enable(poll_time_fast, PERIOD_TIME);
		} else {
			SPRDBAT_DEBUG("polling norm %d!\n", poll_time);
			sprdbat_data->battery_work_cycle_s = poll_time;
			sprdchg_timer_op->timer_disable();
			sprdchg_timer_op->timer_enable(poll_time, PERIOD_TIME);
		}
	}
		...
		const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_INPUT_CURRENT_LIMIT:
...

		{
			...
			/* thermald: set limit 0 means recover to default max */
			SPRDBAT_DEBUG("set chgr limit:%d\n", val->intval);
			if ((limit <= 0)
			    || limit > data->bat_info.chg_current_type_limit)
				limit = data->bat_info.chg_current_type_limit;
			sprd_ext_ic_op->set_input_cur_limit(limit);
			data->bat_info.input_cur_limit = limit;
			SPRDBAT_DEBUG("actual set chgr limit:%d\n", data->bat_info.input_cur_limit);
		}
		break;
	case POWER_SUPPLY_PROP_BATTERY_CHARGING_ENABLED:
	...
	case POWER_SUPPLY_PROP_CHARGING_ENABLED:
		...
		if (val->intval == 0) {
			if (sprd_ext_ic_op->set_input_cur_limit == NULL)
				...
				/* Do not save input_cur_limit, it cause bug when re-enable
				data->bat_info.input_cur_limit = 0; */
				SPRDBAT_DEBUG("Disable charging!\n");
			}
			sprdbat_change_module_state(SPRDBAT_CHG_FORCE_STOP_E);
		} else {
			if (sprd_ext_ic_op->set_input_cur_limit == NULL)
				...
				int limit = data->bat_info.input_cur_limit;

				sprd_ext_ic_op->set_input_cur_limit(limit);

				SPRDBAT_DEBUG("charging enabled: %d, set chgr limit:%d\n",
					val->intval, limit);
			}
			sprdbat_change_module_state(SPRDBAT_CHG_FORCE_START_E);
		}
	...
	case POWER_SUPPLY_PROP_SET_SHIP_MODE:
		...
		if (val->intval == 0) {
			has_set_shipmode = true;
			SPRDBAT_DEBUG("set shipmode! power will be cutoff 10s later!\n");
		} else
			has_set_shipmode = false;
					...
					const char *buf, size_t count)
{
	...
	case FORCE_INPUT_CUR:
		SPRDBAT_DEBUG("force set chgr limit=%d\n", set_value);
		if (set_value <= 0)
			set_value = sprdbat_data->bat_info.chg_current_type_limit;
		if (sprd_ext_ic_op->set_input_cur_limit)
...
static void sprdbat_info_init(struct sprdbat_drivier_data *data)
{
	...
	sprdbat_data->battery_work_cycle_s = sprdbat_data->pdata->chg_polling_time;
...
static int sprdbat_start_charge(void)
{
	...
	if (sprd_ext_ic_op->set_input_cur_limit) {
		unsigned int limit =
			sprdbat_data->bat_info.input_cur_limit;
		sprd_ext_ic_op->set_input_cur_limit(limit);
		SPRDBAT_DEBUG("sprdbat_start_charge, set chgr limit=%d\n", limit);
	}
...
static void sprdbat_change_module_state(uint32_t event)
{
	...
	case SPRDBAT_OTP_COLD_RESTART_E:
		_sprdbat_clear_stopflags((uint32_t)
			SPRDBAT_CHG_END_OTP_COLD_BIT);
		break;
	case SPRDBAT_CHGR_COOL_E:
		sprdbat_data->bat_info.bat_health =
			    POWER_SUPPLY_HEALTH_COOL;
		break;
	case SPRDBAT_CHGR_WARM_E:
		sprdbat_data->bat_info.bat_health =
	...
	case SPRDBAT_CHG_TIMEOUT_E:
		if (sprdbat_data->bat_info.bat_health == POWER_SUPPLY_HEALTH_GOOD ||
			sprdbat_data->bat_info.bat_health == POWER_SUPPLY_HEALTH_WARM ||
			sprdbat_data->bat_info.bat_health == POWER_SUPPLY_HEALTH_COOL)
				sprdbat_data->bat_info.bat_health =
					POWER_SUPPLY_HEALTH_SAFETY_TIMER_EXPIRE;
		sprdbat_data->bat_info.chg_this_timeout =
		    sprdbat_data->pdata->chg_rechg_timeout;
...
static void sprdbat_battery_works(struct work_struct *work)
{
	...
	queue_delayed_work(system_power_efficient_wq,
			   ...
			   sprdbat_data->battery_work_cycle_s * HZ);
}
...
#ifdef ZTE_CHGR_LIMIT_1700mA
	pdata->adp_cdp_cur_limit = 1700;
	pdata->adp_dcp_cur_limit = 1700;
	SPRDBAT_DEBUG("ZTE_CHGR_LIMIT_1700mA defined\n");
#endif
