...
static int sprd_no_support_ratio_cal(struct sprd_thermal_zone *pzone)
{
	int cal_offset = 0;
#if defined(CONFIG_OTP_SPRD_AP_EFUSE) || defined(CONFIG_OTP_SPRD_AP_IEFUSE)
	u32 data = sprd_ap_efuse_read(pzone->dsen_para->cal_efuse_blk);
	...
	if (data == 0)
		...
		cal_offset = (data >> pzone->dsen_para->cal_efuse_bit) & CAL_OFFSET_MASK;
...
{
	switch (pzone->dsen_para->algor_ver) {
	case 0:
	...
	case 1:
		...
		return 0;
	case 2:
		pzone->dsen_para->sensor_cal = sprd_no_support_ratio_cal;
		return 0;
	default:
