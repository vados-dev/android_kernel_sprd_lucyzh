...
{
	return xfrm6_dec_tcpdump_opt ? 1 : 0;
}
...
int __net_init nf6_vowifi_dec_init(struct net *net)
{
#ifdef CONFIG_PROC_FS
	pr_info("create control for dump decoding ipsec pkt.\n");
	if (!proc_create("nf_xfrm_dec_tcpdump6", S_IRUGO | S_IWUGO,
...
void nf6_vowifi_dec_exit(struct net *net)
{
#ifdef CONFIG_PROC_FS
...
static int __init ip6table_vowifi_ipsec_dec_dump_init(void)
{
	...
	err = nf_register_hooks(nf_ipv6_ipsec_dec_dump_ops,
				ARRAY_SIZE(nf_ipv6_ipsec_dec_dump_ops));
	if (err < 0)
		return err;

	ret = register_pernet_subsys(&nf6_net_xfrm_dec_ops);
...
{
	nf_unregister_hooks(nf_ipv6_ipsec_dec_dump_ops,
			    ARRAY_SIZE(nf_ipv6_ipsec_dec_dump_ops));
